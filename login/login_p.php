    ...
    <div class="row dashboard">
        <div class="span3">
            ...
            if (!$_SESSION['auth'] || !$_SESSION['screen_name']) {
                die("<sccript>window.location='{$userHostName}/user/login'</script>");
            }
            ...
            if ($checkDb->success) {
                echo "<a href='http://{$prefix}{$domain}/store/{$_SESSION['screen_name']}' id='btn-admin' class='btn btn-large btn-info' title='' data-placement='top' data-content='Click here to start the geocloud administration.'>Start admin</a>";
            } else {
                echo "<a href='http://{$prefix}{$domain}/user/createstore' id='btn-admin' class='btn btn-large btn-info' title='' data-placement='top' data-content='Click here to create your geocloud. It may take some secs, so stay on this page.'>Create new database</a>";
            }
            ?>
        </div>
        <div class="span3">
